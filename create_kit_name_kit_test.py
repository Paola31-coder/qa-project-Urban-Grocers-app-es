...
def test2_create_kit_511_letter_in_the_name_succes_response():
...
def test5_create_kit_special_characters_in_the_name_succes_reponse():
    current_kit_body = get_kit_body(data.test_5_kit_name)
    positive_assert(current_kit_body)

def test6_create_kit_with_spaces_in_the_name_succes_reponse():
    current_kit_body = get_kit_body(data.test_6_kit_name)
    positive_assert(current_kit_body)

def test7_create_kit_with_numbers_in_the_name_succes_reponse():
    current_kit_body = get_kit_body(data.test_7_kit_name)
    positive_assert(current_kit_body)

def test8_create_kit_without_parameter_in_the_name():
    current_kit_body = data.kit_body.copy()
    current_kit_body.pop("name")
    negative_assert_code_400(current_kit_body)

def test9_create_kit_parameter_number_in_the_name():
    current_kit_body = get_kit_body(data.test_9_kit_name)
    negative_assert_code_400(current_kit_body)